...
def data():
    ...
    sql_statement = "SELECT timestamp, bytes_used, did_aww_snap, current_page FROM reports;"

    for row in curs.execute(sql_statement):
    ...
    # if didCrash is on the row after the actual crash, apply value to previous row, where crash actually happened instead
    crashed_data = []
    for idx, data in enumerate(filtered_data):
        if data["didCrash"]:
            mod_data = filtered_data[idx - 1]
            mod_data['didCrash'] = 1
            crashed_data.append(mod_data)

            filtered_data[idx]['didCrash'] = 0
